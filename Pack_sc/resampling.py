...
def get_proline_chi_bins(
    ...
    SC_D_bin = torch.div(
        SC_D + torch.pi,
        2 * torch.pi / (batch["chi_logits"].shape[-1] - 1),
        rounding_mode="floor",
    )
...
def resample_clashes(
    ...
    aatype_chi_mask = torch.tensor(
        chi_mask_atom14, dtype=torch.float32, device=chi_pred.device
    )[resampled_S]
    chi_pred = aatype_chi_mask * chi_pred
    resampled_atom14_xyz = get_atom14_coords_infer(
