...
def get_proline_chi_bins(batch, atom14_pred_positions, proline_indices=None) -> torch.Tensor:
    if proline_indices is None:
        ...
        SC_D, _ = calc_sc_dihedrals(atom14_pred_positions.squeeze()[pro_mask], batch["S"].squeeze()[pro_mask])
    else:
        # Get the proline chi values and bins
...
def resample_clashes(batch, atom14_pred_positions, clashing_indices, temperature=0.0) -> torch.Tensor:
    ...
    aatype_chi_mask = torch.tensor(chi_mask_atom14, dtype=torch.float32, device=chi_pred.device)[resampled_S]
    chi_pred = aatype_chi_mask * chi_pred
    resampled_atom14_xyz = get_atom14_coords_infer(resampled_X, resampled_S, resampled_BB_D, chi_pred)
    ...
    for pro_i in range(len(proline_indices)):
        ...
        while not replaced and attempts < max_attempts:
            ...
            if repeated_chis:
                ...
                chi_pred = aatype_chi_mask * chi_pred
                resampled_atom14_xyz = get_atom14_coords_infer(resampled_X[pro_i], resampled_S[pro_i], resampled_BB_D[pro_i], chi_pred)
