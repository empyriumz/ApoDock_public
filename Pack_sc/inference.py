...
from Packnn import Pack
from resampling import resample_loop
from data_utils import parse_PDB, get_clean_res_list,featurize, calc_bb_dihedrals,load_model_dict, get_atom14_coords, write_full_PDB
from sc_utils import make_torsion_features, map_mpnn_to_af2_seq
from dataset_pack import mol2graph, res2pdb, get_protein_mpnn_encoder_features, Pocket_BB_D
...
def parse_pockets(pdb_files):
    ...
    for pdb_file in pdb_files:
        ...
        atoms = [atom for residue in structure.get_residues() for atom in residue.get_atoms()]
...
def cluster_pockets(pdb_files, num_clusters=6):
    ...
    clustering = KMedoids(n_clusters=num_clusters, metric='precomputed').fit(rmsd_matrix)
...
def get_graph_data_p(pocket_path, protein_path, ligandmpnn_path, apo2holo=False):
    ...
    # print(pocket_path)
    parser = PDBParser(QUIET=True)
    res_list = get_clean_res_list(parser.get_structure(pdbid, pocket_path).get_residues(),verbose=False, ensure_ca_exist=True)
    res_list_pdb_dir = os.path.join(base_dir,
                            f'Pocket_clean_{pdbid}.pdb'
                            )
...
def get_complex_data(list_of_ligands, list_of_pockets, list_of_proteins, ligandmpnn_path, apo2holo=False):

    garph_data_list_l = []
    graph_data_list_p = []
    for ligand, pocket, protein in zip(list_of_ligands, list_of_pockets, list_of_proteins):
        # print(ligand)
...
def get_file_list(data_dir):
    ...
    for pdbid in pdb_files:
        ...
        or i.endswith("_ligand.mol") or i.endswith("_ligand.mol2") or i.endswith("_ligand.pdb") or i.endswith("_aligned.sdf")]
...
class Pack_infer(Dataset):  
        ...
        def __init__(self, ligand_list, pocket_list, protein_list, ligandmpnn_path, apo2holo=False):
        ...
        def _pre_process(self):
            ...
            garph_data_list_l, graph_data_list_p = get_complex_data(ligand_list, pocket_list, protein_list, ligandmpnn_path, apo2holo)
...
def infer(model, dataloader, device, temperature=1.0, n_recycle=3, resample=True, resample_args={}):
    model.eval()
...
def get_letter_codes(pocket_list):
    ...
    CA_icodes = [np.pad(arr, (0, max_len - len(arr)), mode='constant', constant_values='') for arr in CA_icodes_list]
    Chain_letters = [np.pad(arr, (0, max_len - arr.size), mode='constant', constant_values='') for arr in Chain_letters_list]

    return CA_icodes, Chain_letters
            

def sample_xyz(ligand_list,
            pocket_list,
            protein_list,
            model,
            device,
            ...
            temperature,
            n_recycle,
            ligandmpnn_path,apo2holo=False):
    # ligand_list, pocket_list = get_file_list(data_dir)
    dataset = Pack_infer(ligand_list, pocket_list, protein_list, ligandmpnn_path, apo2holo)
    ...
    for _ in range(number_of_packs_per_design):
        results = infer(model, dataloader, device, temperature, n_recycle, resample=True, resample_args={})
        results_list.append(results)
...
def pro2feature_dict(data_aa):
    X,X_mask, seq,tor_an_gt,chain_label,R_idx, aa_type, ProteinMPNN_feat,BB_D = data_aa.x, data_aa.x_mask, \
    data_aa.seq, data_aa.tor_an_gt, data_aa.chain_label, data_aa.R_idx, data_aa.aa_type, data_aa.ProteinMPNN_feat, data_aa.BB_D
...
def write_pdbs(
    ...
    out_dit_list = [out_dir[i:i + batch_size] for i in range(0, len(out_dir), batch_size)]
    packed_files_list = []
    ...
    for i, results in enumerate(results_list):
        ...
        for pdbids, result, CA_icodes, Chain_letters, out_dirs in zip(pdbid_list, results, CA_icodes_list, Chain_letters_list, out_dit_list):
    ...
    pdbids_packed = list(set([os.path.basename(i).split("_pack_")[0] for i in packed_files_list]))
    packed_files_list = [[i for i in packed_files_list if os.path.basename(i).split("_pack_")[0] == pdbid] for pdbid in pdbids_packed]
