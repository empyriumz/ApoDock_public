...
def rank_docking_results(docked_sdfs, mdn_scores, top_k=10, packing=True, docking_program="smina"):
    ...
    # print(len(mol_list))
    if len(mol_list) == 0:
        print("No molecules were read from the SDF files or pass the rdkit sanitization")
        return None
    ...
    sorted_pairs = sorted(zip(mdn_scores, mol_list, mol_name_list), key=lambda pair: pair[0], reverse=True)
...
def vina_dock(
    ...
    if packing:
        ...
        if docking_program == "smina.static":
            output_dir = os.path.join(out_dir, f"{protein_id.split('.')[0]}_smina_dock_{ligand_id}.sdf")
        elif docking_program == "gnina":
            output_dir = os.path.join(out_dir, f"{protein_id.split('.')[0]}_gnina_dock_{ligand_id}.sdf")
    else:
...
def parse_args():
    ...
    parser.add_argument("--csv", type=str, help="Path to the CSV file containing the ligand and protein paths")
    parser.add_argument("--protein", type=str, help="Path to the directory containing the proteins")
    parser.add_argument("--ligand", type=str, help="Path to the directory containing the ligands")
    parser.add_argument("--ref_lig", type=str, default=None, help="Path to the reference ligand")
    parser.add_argument("--ckpt_sc", type=str, default="./checkpoints/ApoPack_time_split_0.pt", help="Path to the ckpt of the SC model")
    parser.add_argument("--ckpt_mdn", type=str, default="./checkpoints/ApoScore_time_split_0.pt", help="Path to the ckpt of the MDN model")
    parser.add_argument("--packs_per_design", type=int, default=40, help="Number of packs per design")
    parser.add_argument("--docking_program", type=str, default="gnina", help="Docking program to use")
    parser.add_argument("--packing", action="store_true", help="Whether to pack the protein")
    parser.add_argument("--packing_batch", type=int, default=16, help="Batch size for packing")
    parser.add_argument("--num_clusters", type=int, default=6, help="Number of clusters to use for packing")
    parser.add_argument("--ligandmpnn_path", type=str, default="./checkpoints/proteinmpnn_v_48_020.pt", help="Path to the ligand MPNN model")
    parser.add_argument("--num_modes", type=int, default=40, help="Number of modes to use for docking")
    parser.add_argument("--exhaustiveness", type=int, default=32, help="Exhaustiveness to use for docking")
    parser.add_argument("--device", type=str, default="cuda", help="Device to use for docking")
    parser.add_argument("--top_k", type=int, default=40, help="Number of top poses to keep")
    parser.add_argument("--autobox_add", type=float, default=6.0, help="Autobox add value")
    parser.add_argument("--temperature", type=float, default=2, help="Temperature value for packing")
...
def main():
    ...
    assert len(ligand_list) == len(protein_list) == len(ref_lig_list), "Data counts do not match"
